...
final readonly class LabelOutputManager
{
    ...
    public function generate(LabelOutputFormat $format, LabelOptions $options, array $elements, string $filenameBase, int $copies = 1, int $startSlot = 1): GeneratedLabelFile
    {
        foreach ($this->generators as $generator) {
            if ($generator->supports($format)) {
                return $generator->generate($options, $elements, $filenameBase, $copies, $startSlot);
            }
        }
